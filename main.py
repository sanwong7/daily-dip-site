...
def auto_select_candidates():
    ...
    try:
        ...
        if spy.empty: spy_returns = []
        else: spy_returns = spy['Close'].pct_change().dropna()
    except: spy_returns = []
    
    print(f"🔍 開始過濾...")
    for ticker in full_list:
        try:
            # 暫時不隨機延遲，因為可能導致超時，若需要可加回
            ...
            if dollar_vol < 50_000_000: continue 
...
def clean_data_for_json(obj):
    ...
    if isinstance(obj, np.floating): 
        ...
        return val
    if isinstance(obj, np.ndarray): return clean_data_for_json(obj.tolist())
    if isinstance(obj, dict): return {k: clean_data_for_json(v) for k, v in obj.items()}
    if isinstance(obj, list): return [clean_data_for_json(v) for v in obj]
    return obj
...
def main():
    ...
    # 🔥 關鍵：清理數據後再進行 Base64 編碼
    clean_app_data = clean_data_for_json(APP_DATA)
    json_str = json.dumps(clean_app_data)
