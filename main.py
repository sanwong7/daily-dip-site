...
def calculate_beta(stock_returns, market_returns):
    # 如果沒有大盤數據，默認 Beta = 1.2 (避免被過濾)
    if len(market_returns) == 0: return 1.2
...
def auto_select_candidates():
    ...
    try:
        ...
        if not spy.empty:
            ...
            print("⚠️ SPY data empty. Proceeding without Beta filter.")
    ...
    for ticker in full_list:
        try:
            ...
            # 暫時放寬 200MA 過濾，確保有東西顯示
            # if close < sma200: continue 
            
            avg_vol = df['Volume'].tail(30).mean()
            # 放寬流動性限制
            if (avg_vol * close) < 50_000_000: continue 
...
def get_market_condition():
    try:
        session = get_session()
        print("🔍 Checking Market Condition...")
        spy = yf.Ticker("SPY", session=session).history(period="6mo")
        
        if spy.empty: 
            # 如果抓不到 SPY，默認給 BULLISH，避免全站壞掉
            return "BULLISH", "🟢 市場順風 (Default)", 5
    ...
    except: return "BULLISH", "🟢 市場順風 (Fallback)", 5 # 出錯也默認 Bullish
...
def main():
    ...
    .macro-grid {{ display:grid; grid-template-columns: repeat(4, 1fr); gap:10px; margin-bottom:15px; height: 150px; }}
